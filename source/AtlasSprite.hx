...
class AtlasSprite extends FlxAnimate
{
	...
	public function loadAtlas(_path:String, ?_settings:FlxAnimateSettings)
	{
		...
		if (Assets.exists(_path + "/spritemap1.png"))
		{
			...
			if ((fromMod != null
				&& PolymodHandler.getSeparatedVersionNumber(PolymodHandler.getModMetaFromFolder(fromMod).api_version)[1] <= 7))
			{ // API version that old atlas stuff uses.
				isOld = true;

				applyStageMatrix = true;
				origin.set(-timeline.getBoundsOrigin().x,
					-timeline.getBoundsOrigin().y); // Scales from the origin of the symbol instead of the center of the sprite.
	...
	public function addAnimationByLabel(name:String, label:String, ?framerate:Float = 24, ?looped:Bool = false, ?loopFrame:Null<Int> = null):Void
	{
		...
		if (isOld)
		{
			...
			var length:Int = (labelIndex < frameLabelInfo.length - 1) ? frameLabelInfo[labelIndex + 1].index
				- frameLabelInfo[labelIndex].index : anim.getByName("___full").frames.length
				- frameLabelInfo[labelIndex].index;
	...
	public function addAnimationStartingAtLabel(name:String, label:String, length:Null<Int>, ?framerate:Float = 24, ?looped:Bool = false,
			?loopFrame:Null<Int> = null):Void
	{
		var foundFrames = anim.findFrameLabelIndices(label);
		if (foundFrames.length <= 0)
		{
	...
	function onFrameChange(name:String, frame:Int, index:Int):Void
	{
		...
		{
			frameCallback(curAnim, frame - animInfo.startFrame, frame);
		}
	...
	private function animationEndBehavior(animInfo:AtlasAnimInfo)
	{
		...
		anim.play(anim.curAnim.name, true, false,
			(animInfo.startFrame +
				animInfo.length) - 1); // Prevents the game from potentially showing the next frame of animation if the game lags right before the animation ends.
		anim.pause();
	...
	public static function loadAndCache(animate:String, ?unique:Bool = false, ?settings:FlxAnimateSettings):FlxAnimateFrames
	{
		...
		{
			if (!f.startsWith("spritemap"))
			{
				spritemapList.remove(f);
			}
		}
