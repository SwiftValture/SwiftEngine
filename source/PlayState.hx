...
import haxe.display.Server.ContextParams;
...
import shaderslmfao.AdjustColorShader;
...
class PlayState extends MusicBeatState
{
	...
	private var strumBaseAlpha:Array<Float> = [];

	override public function create()
	{
		FlxG.sound.cache(Paths.sound('scrollMenu'));
		FlxG.sound.cache(Paths.music('breakfast'));
		FlxG.sound.cache(Paths.sound('hitSound')); // hab ich von codename engine gestohlen lol. muss ich noch ersetzen
		...
		cpuControlled = PreferencesMenu.getPref('botplay'); // ich hab den scheiss lowkey irgendwo reingequetscht
		...
		switch (SONG.song.toLowerCase())
		{
			case 'spookeez' | 'monster' | 'south' | 'south-erect' | 'spookeez-erect':
				curStage = "spooky";
				halloweenLevel = true;
			...
			case 'pico-erect' | 'blammed-erect' | 'philly-erect':
				defaultCamZoom = 1.1;

				curStage = 'philly-erect';

				var bg:FlxSprite = new FlxSprite(-100, 0);
				bg.loadGraphic(Paths.image('philly/erect/sky'));
				bg.scrollFactor.set(0.1, 0.1);
				bg.antialiasing = true;
				add(bg);

				var city:FlxSprite = new FlxSprite(-255, 45);
				city.loadGraphic(Paths.image('philly/erect/city'));
				...
				light = new FlxSprite(-255, 45).loadGraphic(Paths.image('philly/erect/win'));
				light.scrollFactor.set(0.3, 0.3);
				light.visible = false;
				...
				var streetBehind:FlxSprite = new FlxSprite(-299, 144);
				streetBehind.loadGraphic(Paths.image('philly/erect/behindTrain'));
				streetBehind.scrollFactor.set(1, 1);
				streetBehind.antialiasing = true;
				add(streetBehind);

				phillyTrain = new FlxSprite(2000, 360);
				phillyTrain.loadGraphic(Paths.image('philly/train'));
				phillyTrain.scrollFactor.set(1, 1);
				phillyTrain.antialiasing = true;
				add(phillyTrain);

				var street:FlxSprite = new FlxSprite(-299, 144);
				street.loadGraphic(Paths.image('philly/erect/street'));
				street.scrollFactor.set(1, 1);
			...
			case "milf" | 'satin-panties' | 'high' | 'high-erect' | 'satin-panties-erect':
				curStage = 'limo';
				defaultCamZoom = 0.90;
			...
			case 'bopeebo-erect' | 'fresh-erect' | 'dadbattle-erect':
				...
				crowd.frames = Paths.getSparrowAtlas('stage/erect/crowd');
				...
				brightLightSmall.loadGraphic(Paths.image('stage/erect/brightLightSmall'));
				...
				lightred.loadGraphic(Paths.image('stage/erect/lightred'));
				...
				lightAbove.loadGraphic(Paths.image('stage/erect/lightAbove'));
			...
			case 'darnell-boyfriend' | 'lit-up-boyfriend':
				...
				var rainShaderIntensity = 0.0;
				if (SONG.song.toLowerCase() == 'lit-up-boyfriend')
				{
					rainShaderIntensity = 0.02;
				}
				else if (SONG.song.toLowerCase() == 'darnell-boyfriend')
				{
					rainShaderIntensity = 0.01;
				}

				rainShader = new RainShader();
				rainShader.uRainColor.value = getColorVec(rainColor);
				rainShader.uScale.value = [FlxG.height / 200];
				rainShader.uIntensity.value = [rainShaderIntensity];
	...
	public function resumeTrainSound():Void
	{
		if (trainSound != null)
			trainSound.resume();
	}
	...
	private function generateSong():Void
	{
		...
		if (SONG.needsVoices)
		{
				...
				|| SONG.song.toLowerCase() == 'lit-up-boyfriend')
			{
				...
				if (openfl.utils.Assets.exists(oppVoices))
					vocals = new FlxSound().loadEmbedded(oppVoices);
				else
					vocals = new FlxSound();

				if (openfl.utils.Assets.exists(playerVoices))
					playerVocals = new FlxSound().loadEmbedded(playerVoices);
				else
		...
		playerVocals.onComplete = function()
		{
	...
	private function playStrumAnim(strum:FlxSprite, anim:String, force:Bool = false):Void
	{
		if (strum == null)
			return;
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			strumBaseY.push(babyArrow.y);
			strumBaseAngle.push(babyArrow.angle);
			strumBaseAlpha.push(babyArrow.alpha);
	...
	override public function update(elapsed:Float)
	{
		...
		if (rainShader != null)
		{
			rainShader.uCameraBounds.value = [camGame.viewLeft, camGame.viewTop, camGame.viewRight, camGame.viewBottom];
			rainShader.uTime.value = [time];
		}

		rainTimeScale = smoothLerpPrecision(rainTimeScale, 0.02, elapsed, 2.5);
		...
		switch (curStage)
		{
			...
			case 'philly-erect':
				...
				var curBeatFloat:Float = Conductor.songPosition / Conductor.crochet;
		...
		var currentBeat = ((Conductor.getTimeWithDelta() / 1000) * (Conductor.bpm / 60)) * 0.1;

		if (PreferencesMenu.getPref('Test Modchart'))
		{
			for (i in 0...4)
			{
				var n = strumLineNotes.members[i];

				if (n == null)
					continue;

				n.x = strumBaseX[i] - 300 * Math.sin(currentBeat * 8) + 350;

				n.y = strumBaseY[i] + 64 * Math.cos((currentBeat * 8 + i * 5) * Math.PI) + 10;
			}

			for (i in 0...4)
			{
				var index = i + 4;
				var n = strumLineNotes.members[index];

				if (n == null)
					continue;

				n.x = strumBaseX[index] + 300 * Math.sin(currentBeat * 8) - 275;

				n.y = strumBaseY[index] - 64 * Math.cos((currentBeat * 8 + index * 5) * Math.PI) + 10;
			}
		}
		...
		for (i in 0...4)
		{
			...
			if (strum != null && strum.animation.curAnim != null)
			{
		...
		if (controls.PAUSE && startedCountdown && canPause)
		{
			...
			if (trainSound != null && trainMoving)
				trainSound.pause();
		...
		if (camZooming)
		{
			var factor:Float = Math.pow(0.95, elapsed * 60);

			FlxG.camera.zoom = FlxMath.lerp(defaultCamZoom, FlxG.camera.zoom, factor);
		...
		if (generatedMusic)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (daNote.mustPress)
				{
					if (cpuControlled && daNote.canBeHit && (daNote.isSustainNote || daNote.strumTime <= Conductor.songPosition))
						goodNoteHit(daNote);
	...
	private function keyShit():Void
	{
		...
		if (!cpuControlled)
		{
			...
			else if (boyfriend.holdTimer > Conductor.stepCrochet * boyfriend.holdTime * 0.001)
			{
		...
		playerStrums.forEach(function(spr:FlxSprite)
		{
			...
			if (spr.animation.curAnim.name == 'confirm' && !curStage.startsWith('school'))
			{
				...
				var offsetX = spr.ID == 1 ? 16 : 13;
				var offsetY = 13;

				spr.offset.x -= offsetX;
				spr.offset.y -= offsetY;
			}
			else
			{
				spr.centerOffsets();
		...
		for (i in 0...4)
		{
			var spr = strumLineNotes.members[i];
	...
	{
		var strum = strumLineNotes.members[note.noteData];
	...
	override function stepHit()
	{
		...
		if (songEvents != null)
		{
			var stepHit:Dynamic = songEvents.variables.get('stepHit');

			if (stepHit != null)
			{
				Reflect.callMethod(null, stepHit, [curStep]);
			}
		}
	...
	override function beatHit()
	{
		...
		if (songEvents != null)
		{
			var beatHit:Dynamic = songEvents.variables.get('beatHit');

			if (beatHit != null)
			{
				Reflect.callMethod(null, beatHit, [curBeat]);
			}
		}
